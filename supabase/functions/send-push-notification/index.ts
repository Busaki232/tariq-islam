...
async function signJwtRS256(privateKeyPem: string, header: unknown, payload: unknown) {
...
function buildCallUrlFromMetadata(meta: any) {
  ...
  const callerId = meta?.from_user_id || meta?.caller_id || meta?.callerId || "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fcmErrors: Array<{ notifId: string; status: number; body: string }> = [];
        ...
        const message = {
          message: {
            ...
            data: {
              ...
              conversationId: String(meta?.conversation_id || meta?.conversationId || ""),
              callerId: String(meta?.from_user_id || meta?.caller_id || meta?.callerId || ""),
              callerName: String(meta?.caller_name || meta?.callerName || ""),
            },
            android: {
              priority: "HIGH",
              ttl: "30s",
