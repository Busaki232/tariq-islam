  ...
  const joinLivestream = useCallback(async () => {
    ...
    try {
      ...
      call.on("joined-meeting", () => {
        console.log(
          "[ScholarLivestream] joined-meeting"
        );
      });

      call.on("left-meeting", () => {
        console.log(
          "[ScholarLivestream] left-meeting"
        );
      });
      ...
      call.on("camera-error", (event: any) => {
        console.error(
          "[ScholarLivestream] Camera error:",
          event
        );
      });

      console.log(
        "[ScholarLivestream] Starting Daily join",
        {
      ...
      const joinResult = await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          window.setTimeout(() => {
            reject(
              new Error(
                "Daily connection timed out after 20 seconds."
              )
            );
          }, 20000);
        }),
      ...
      console.log(
        "[ScholarLivestream] Join result:",
        joinResult
      );

      console.log(
        "[ScholarLivestream] Meeting state:",
        call.meetingState()
      );
      ...
      console.error(
        "[ScholarLivestream] Unable to join:",
        joinError
      );
