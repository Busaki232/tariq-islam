...
export default function FeaturedReflection() {
  ...
  const [captionTranslations, setCaptionTranslations] =
    ...
    const [featuredVideoTime, setFeaturedVideoTime] = useState(0);
    const [
      selectedFeaturedCaptionLanguage,
      setSelectedFeaturedCaptionLanguage,
    ] = useState("original");
    const [featuredCaptionsCollapsed, setFeaturedCaptionsCollapsed] =
      useState(false);
...
.select(
  ...
  useEffect(() => {
    ...
    setSelectedFeaturedCaptionLanguage("original");
    setFeaturedCaptionsCollapsed(false);
  }, [featured?.id]);

  const originalFeaturedCaptionSegments: CaptionSegment[] =
    featured && Array.isArray(featured.captions_segments)
      ? featured.captions_segments
          .map((segment, index) => ({
            id:
  ...
  const featuredTranslation =
    featured &&
    selectedFeaturedCaptionLanguage !== "original" &&
    selectedFeaturedCaptionLanguage !== "off"
      ? (captionTranslations[featured.id] ?? []).find(
          (translation) =>
  ...
  const featuredCaptionSegments: CaptionSegment[] =
    selectedFeaturedCaptionLanguage === "off"
      ? []
      : selectedFeaturedCaptionLanguage === "original"
        ? originalFeaturedCaptionSegments
        : Array.isArray(featuredTranslation?.translated_segments)
          ? featuredTranslation.translated_segments
          : [];

  const activeFeaturedCaption =
    featuredCaptionSegments.find((segment) => {
      const start = Number(segment.start);
      const end = Number(segment.end);

      return (
        featuredVideoTime >= start &&
        featuredVideoTime < end
      );
    })?.text?.trim() ?? "";

  const featuredHasCaptions =
    originalFeaturedCaptionSegments.length > 0 ||
    Boolean(
      featured &&
        (captionTranslations[featured.id] ?? []).some(
          (translation) =>
...
onTimeUpdate={(event) => {
  ...
  setFeaturedVideoTime(video.currentTime);
  ...
  <div className="absolute bottom-4 left-3 right-3 z-30">
    <div className="overflow-hidden rounded-xl border border-white/20 bg-black/80 text-white shadow-xl backdrop-blur-md">
      <div className="flex items-center gap-2 border-b border-white/10 p-2">
        <select
          value={selectedFeaturedCaptionLanguage}
          onChange={(event) => {
            setSelectedFeaturedCaptionLanguage(
              event.target.value
            );
            setFeaturedVideoTime(
              featuredVideoTime
            );
          }}
          ...
          aria-label={t("reflections.captionLanguage", {
            defaultValue: "Caption language",
          })}
        >
          <option value="off">
            {t("reflections.captionsOff", {
          ...
          {originalFeaturedCaptionSegments.length > 0 && (
            <option value="original">
              {t("reflections.original", {
                defaultValue: "Original",
              })}
            </option>
          )}

          {(captionTranslations[featured.id] ?? []).map(
            (translation) => (
              <option
        ...
        <button
          ...
          onClick={(event) => {
            ...
            setFeaturedCaptionsCollapsed(
              (current) => !current
            );
          }}
          className="rounded-md p-1 text-white/80 hover:bg-white/10"
          aria-label={
            featuredCaptionsCollapsed
              ? t("reflections.expandCaptions", {
                  defaultValue: "Expand captions",
                })
              : t("reflections.collapseCaptions", {
      ...
      {!featuredCaptionsCollapsed &&
        selectedFeaturedCaptionLanguage !== "off" && (
          <div
            dir={
              selectedFeaturedCaptionLanguage === "ar" ||
              selectedFeaturedCaptionLanguage === "ur"
                ? "rtl"
                : "auto"
            }
            className="max-h-28 overflow-y-auto px-3 py-2 text-center text-sm font-medium leading-relaxed sm:text-base"
          >
            {activeFeaturedCaption || (
              <span className="text-white/50">
                {t("reflections.captionsWillAppear", {
                  defaultValue:
