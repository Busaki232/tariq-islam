...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (livestream.status === "ended" || livestream.ended_at) {
      return jsonResponse(
        { error: "This livestream has ended." },
        410,
      );
    }
    ...
    const isBroadcaster =
      isScholarOwner || isAdminOrModerator;

    const nowSeconds = Math.floor(Date.now() / 1000);
    const tokenExpiration = nowSeconds + 6 * 60 * 60;

    const tokenProperties = isBroadcaster
      ? {
    ...
    console.error(
      "Unexpected livestream token error:",
      error,
    );
