...
const [selectedCaptionLanguage, setSelectedCaptionLanguage] =
  ...
  const [captionMenuOpen, setCaptionMenuOpen] =
    useState(false);
     ...
     {hasCaptionChoices && (
       <div className="absolute left-3 top-3 z-[60]">
         <button
           type="button"
           onClick={() =>
             setCaptionMenuOpen((current) => !current)
           }
           className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-black/75 text-xl font-bold text-white shadow-lg backdrop-blur-md"
           aria-label={t(
         ...
         {captionMenuOpen && (
           <div className="mt-2 min-w-[180px] overflow-hidden rounded-xl border border-white/15 bg-black/90 p-1 text-white shadow-xl backdrop-blur-md">
             <button
               type="button"
               ...
               onClick={() => {
                 setSelectedCaptionLanguage("off");
             ...
             {originalCaptionSegments.length > 0 && (
               ...
               >
                 {lecture.captions_language
                   ? `Original (${lecture.captions_language})`
                   : "Original"}
               </button>
             )}

             {captionTranslations.map((translation) => (
               <button
          ...
          {activeCaption && (
          <div className="pointer-events-none absolute bottom-14 left-1/2 z-20 w-[calc(100%-32px)] max-w-3xl -translate-x-1/2 text-center">
            <span
              dir={
              ...
              className="inline-block rounded-lg bg-black/80 px-3 py-2 text-sm font-medium leading-relaxed text-white shadow-lg backdrop-blur-sm sm:text-base"
            >
              {activeCaption}
            </span>
          </div>
